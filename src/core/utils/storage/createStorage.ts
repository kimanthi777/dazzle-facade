...
function serializeJSON<T>(value: T, hookName: string) {
  try {
    ...
    throw new Error(`Hook ${hookName}: Failed to serialize the value, @src/core/utils/storage`);
  }
}
...
export function createStorage<T>(type: StorageType, hookName: string) {
  ...
  return function useStorage(
    { key, defaultValue = undefined, getInitialValueInEffect = true, deserialize = deserializeJSON,
    serialize = (value: T) => serializeJSON(value, hookName),}: IStorageProperties<T>
    ) {
